...
class PantallaMisFavoritosTest {
    ...
    fun login() {
        ...
        val bottonNavigation = onView(withId(R.id.bottom_navigation))
        bottonNavigation.perform(click())
    }

    @Test
    fun test1SeeMyFavouriteRestaurants() {
        // Wait a bit
        Thread.sleep(2000)
    ...
    @Test
    fun test2SeeMyFavoriteRestaurantsIfExists() {
        // Wait a bit
        Thread.sleep(2000)

        val hasFavoriteRestaurants = try {
            val rvMyFavorites = onView(
                Matchers.allOf(
                    withId(R.id.recyclerViewRestaurantes),
                    isDisplayed()
                )
            )
            rvMyFavorites.check(matches(isDisplayed()))
            true
        } catch (e: NoMatchingViewException) {
            false
        }
    ...
    @Test
    fun test3SeeNotAnyFavoriteIfDontHaveAny() {
        // Wait a bit
        Thread.sleep(2000)
        ...
        labelEmptyList.check(matches(isDisplayed()))
    }
}